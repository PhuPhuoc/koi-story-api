...
func (store *consultStore) GetMyListConsult(user_id string) ([]consultmodel.PostConsult, error) {
	...
	WHERE
		p.deleted_at is null and pim.deleted_at is null and p.user_id=?
	ORDER BY
		p.created_at DESC
	`

	err := store.db.Select(&result, query, user_id)
	if err != nil {
